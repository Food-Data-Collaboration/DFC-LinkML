...
- **Import returns**: always `SemanticObject[]` (Ruby: Array; TS: `Promise<SemanticObject[]>`). Single `@graph` entry → 1-element array; use `result[0]`. `Array.isArray(result)` is always true for TS.
