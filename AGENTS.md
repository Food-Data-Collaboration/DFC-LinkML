...
| Cross-connector matrix | `python3 tests/cross_connector/run_matrix.py --verify-drop-in` (repo root) | Needs node + ruby + php, official-connector deps, network |
...
- **Predicates are official short-form** from slot `aliases` (e.g. `dfc-b:VATnumber`), never `dfc-b:Class:snake_case`. All generators emit a `PREDICATE_MAP` (predicate → propName) consulted before the local-name fallback on import.
...
- `scripts/generate_php_connector.py` generates `php-connector/` (models + trait/entity interfaces + `Connector.php` + `SemanticObject.php`). PHP has no `ml/json-ld` runtime dep — predicates are already official CURIEs so no compaction step is needed. `scripts/add_enum_shacl.py` maintains enum constraints in `shacl/`.
