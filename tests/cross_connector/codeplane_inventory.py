#!/usr/bin/env python3
"""Code-plane API inventory: official v2 vs ours, TS + Ruby.

Regenerates live API dumps (Node/Ruby subprocesses), verifies
config/dfc-official-api.yaml against them, and renders
...
YAML_PATH = REPO / "config" / "dfc-official-api.yaml"
DOCS = REPO / "docs"
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # --- class coverage: official types must exist in ours ---
        ...
        for entry in slots:
            ...
            for lang, dump, key in (
                ...
                if not spec:
                    continue
                # official classes exercising this canonical (from adapters)
        ...
        # --- unmapped official domain methods on covered classes ---
        covered = set()
...
def render_ts(rows, unmapped, official, ours_fields, type_aliases) -> None:
    lines = [
        "# Code-plane API gaps — TypeScript (official v2 vs ours)",
        "",
        "> Generated by `tests/cross_connector/codeplane_inventory.py` from",
        "> `@datafoodconsortium/connector@2.0.0-beta.2`. Do not hand-edit; "
        "change `config/dfc-official-api.yaml` and re-run.",
        "",
        "Ours uses plain public fields; official uses get/set/add/remove",
        ...
        "## Mapped methods",
        "",
        "| Canonical | Predicate | Official class | Official method | Ours field |",
        "|---|---|---|---|---|",
    ]
    ...
    lines += [
        "## Class coverage",
        "",
        f"Official classes: {len([t for t in official if t.startswith('dfc-b:')])}; "
        f"ours classes: {len(ours_fields)}. "
        "Ours is a superset; `dfc-b:Enterprise` exists only in ours "
        "(deprecated subclass + import alias).",
        "",
    ]
    if type_aliases:
        lines += ["Known spelling counterparts:", ""]
        lines += [f"- `{a}` (official) ↔ `{b}` (ours)" for a, b in
...
def render_rb(rows, unmapped, official, ours_rb, type_aliases) -> None:
    lines = [
        ...
        "",
        "> Generated by `tests/cross_connector/codeplane_inventory.py` from",
        "> `datafoodconsortium-connector >= 2.0.0.pre.beta8`. Do not hand-edit; "
        "change `config/dfc-official-api.yaml` and re-run.",
        "",
